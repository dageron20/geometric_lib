...
Библиотека **math** предоставляет набор функций для выполнения математических, тригонометрических и логарифмических операций. В нашем случае используется для добавления работы с числом **pi**.
```python
import math
```
